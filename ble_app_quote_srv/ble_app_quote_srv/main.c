...
#define DEVICE_NAME                     "QUOTE_SERVER"                           /**< Name of device. Will be included in the advertising data. */
...
static void advertising_init(void)
{
    ...
    ble_uuid_t adv_uuids[] = {{BTS_UUID_SERVICE, BLE_UUID_TYPE_BLE}};
...
void write_read_handler(ble_quote_t *p_bts, uint8_t new_state)
{
	...
	// when ever you write from your central of characteristic 1 or from the dongle side, this is the call back
	// you can implement your logic here for writing and reading from the flash depending on the commands something like that
...
void erase_handler(ble_quote_t *p_bts, uint8_t new_state)
{
	...
	// when ever you write from your central or characteristic 2 or from the dongle side, this is the call back
	// you can implement your logic here for erasing from the flash depending on the commands something like that 
...
static void on_ble_evt(ble_evt_t * p_ble_evt)
{
	...
	//timers_start();  -- no timers are used so not required here
...
int main(void)
{
    ...
    sec_params_init();
     //twi_master_init();
	  //inertial_sensing_init();
		quote_server_init(); // depends on how you are going to use, i just gave empty template for this 
	
	
    // Start execution
    //timers_start();
	  // starting the timers when the device is connected with the other device
    advertising_start();
