...
typedef struct ble_inert_s ble_inert_t;
typedef void (*ble_inert_delay_write_handler_t) (ble_inert_t * p_bts, uint8_t new_state);
...
typedef struct
{
    ble_inert_delay_write_handler_t   accel_delay_write_handler; 
		ble_inert_delay_write_handler_t   magn_delay_write_handler; 
	  ble_inert_delay_write_handler_t   gyro_delay_write_handler; 
					
} ble_inert_init_t;

typedef struct ble_inert_s
{         
    uint16_t                     service_handle;                 
    ble_gatts_char_handles_t     accel_char_handles;   
		ble_gatts_char_handles_t     magn_char_handles;
...
uint32_t ble_inert_init(ble_inert_t * p_inert, const ble_inert_init_t * p_inert_init);


void ble_inert_on_ble_evt(ble_inert_t * p_inert, ble_evt_t * p_ble_evt);
...
typedef struct inertial_sensing
{
	Magnetometer magn;
	Gyroscope gyro;
	Accelerometer accel;
...
void get_data(Inertial_sensing * );
...
void read_accelerometer(Accelerometer *);
...
void read_magnetometer(Magnetometer *);
...
void inertial_sensing_init(void);
...
void analyze_data_inertial(const Inertial_sensing *const  );


void analyze_data_accel(const Accelerometer *);
void analyze_data_gyro(const Gyroscope *);
void analyze_data_magn(const Magnetometer *); 

void accel_delay_write_handler(ble_inert_t *p_bts, uint8_t new_state);
void magn_delay_write_handler(ble_inert_t *p_bts, uint8_t new_state);
void gyro_delay_write_handler(ble_inert_t *p_bts, uint8_t new_state);
...
static void on_connect(ble_inert_t * p_inert, ble_evt_t * p_ble_evt)
{
    p_inert->conn_handle = p_ble_evt->evt.gap_evt.conn_handle;
}

static void on_disconnect(ble_inert_t * p_inert, ble_evt_t * p_ble_evt)
{
    ((void)(p_ble_evt));
    p_inert->conn_handle = 0xFFFF;
}


 

static void on_write(ble_inert_t * p_inert, ble_evt_t * p_ble_evt)
{
    ble_gatts_evt_write_t * p_evt_write = &p_ble_evt->evt.gatts_evt.params.write;
   
	
	if ((p_evt_write->handle == p_inert->accel_char_handles.value_handle) &&
       (p_evt_write->len == 1) &&
       (p_inert->accel_delay_write_handler != 0))
   {
		   p_inert->accel_delay_write_handler(p_inert, p_evt_write->data[0]);
   }
	 if ((p_evt_write->handle == p_inert->magn_char_handles.value_handle) &&
       (p_evt_write->len == 1) &&
       (p_inert->magn_delay_write_handler != 0))
   {
		   p_inert->magn_delay_write_handler(p_inert, p_evt_write->data[0]);
   }
	 
	 if ((p_evt_write->handle == p_inert->gyro_char_handles.value_handle) &&
       (p_evt_write->len == 1) &&
       (p_inert->gyro_delay_write_handler != 0))
   {
		   p_inert->gyro_delay_write_handler(p_inert, p_evt_write->data[0]);
   }
...
void ble_inert_on_ble_evt(ble_inert_t * p_inert, ble_evt_t * p_ble_evt)
{
    switch (p_ble_evt->header.evt_id)
    {
        case BLE_GAP_EVT_CONNECTED:
            on_connect(p_inert, p_ble_evt);
            break;
            
        case BLE_GAP_EVT_DISCONNECTED:
            on_disconnect(p_inert, p_ble_evt);
            break;
            
        case BLE_GATTS_EVT_WRITE:
					  on_write(p_inert, p_ble_evt);
            break;
...
static uint32_t accel_char_add(ble_inert_t * p_inert, const ble_inert_init_t * p_inert_init)
{
    ...
    memset(&char_md, 0, sizeof(char_md));
	  ...
	  char_md.char_props.read = 1;
		...
		ble_uuid.type = p_inert->uuid_type;
    ...
    attr_char_value.max_len      = 25;
    attr_char_value.p_value      = 0;
		
		
		
	return sd_ble_gatts_characteristic_add(p_inert->service_handle, &char_md,
                                               &attr_char_value,
                                               &p_inert->accel_char_handles);
}

static uint32_t magn_char_add(ble_inert_t * p_inert, const ble_inert_init_t * p_inert_init)
{
	ble_gatts_char_md_t char_md;
		ble_gatts_attr_md_t cccd_md;
	  ble_gatts_attr_t attr_char_value;
	...
	return sd_ble_gatts_characteristic_add(p_inert->service_handle, &char_md,
                                               &attr_char_value,
                                               &p_inert->magn_char_handles);
}

static uint32_t gyro_char_add(ble_inert_t * p_inert, const ble_inert_init_t * p_inert_init)
{
	ble_gatts_char_md_t char_md;
		ble_gatts_attr_md_t cccd_md;
	  ble_gatts_attr_t attr_char_value;
...
uint32_t ble_inert_init(ble_inert_t * p_inert, const ble_inert_init_t * p_inert_init)
{
    ...
    p_inert->conn_handle       = 0xFFFF;
		...
		p_inert->accel_delay_write_handler =  p_inert_init->accel_delay_write_handler;
	  p_inert->magn_delay_write_handler =  p_inert_init->magn_delay_write_handler;
	  p_inert->gyro_delay_write_handler =  p_inert_init->gyro_delay_write_handler;
	
    
    ble_uuid128_t base_uuid = {0x23, 0xD1, 0xBC, 0xEA, 0x5F, 0x78, 0x23, 0x15, 0xDE, 0xEF, 0x12, 0x12, 0x00, 0x00, 0x00, 0x00};
    err_code = sd_ble_uuid_vs_add(&base_uuid, &p_inert->uuid_type);
    if (err_code != ((0x0) + 0))
    {
        return err_code;
    }
    ...
    ble_uuid.type = p_inert->uuid_type;
    ble_uuid.uuid = 0x1522;

    err_code = sd_ble_gatts_service_add(0x01, &ble_uuid, &p_inert->service_handle);
    if (err_code != ((0x0) + 0))
    {
        return err_code;
    }
    ...
    err_code = accel_char_add(p_inert, p_inert_init);
    if (err_code != ((0x0) + 0))
   {
        return err_code;
    }
		...
		err_code = magn_char_add(p_inert, p_inert_init);
    if (err_code != ((0x0) + 0))
    {
        return err_code;
    }
		...
		err_code = gyro_char_add(p_inert, p_inert_init);
    if (err_code != ((0x0) + 0))
    {
        return err_code;
    }
...
uint32_t ble_inert_notify_accel(ble_inert_t * p_inert, struct accelerometer *accel_data)
{
	  ble_gatts_hvx_params_t params;
    uint16_t len = 6;			
    memset(&params, 0, sizeof(params));
    params.type = 0x01;
    params.handle = p_inert->accel_char_handles.value_handle;
    params.p_data = (uint8_t *)accel_data;
    params.p_len = &len;
	
	  
		return sd_ble_gatts_hvx(p_inert->conn_handle, &params);
}

uint32_t ble_inert_notify_magn(ble_inert_t * p_inert, Magnetometer *magn_data)
{
	  ble_gatts_hvx_params_t params;
    uint16_t len = 6;			
    memset(&params, 0, sizeof(params));
    params.type = 0x01;
    params.handle = p_inert->magn_char_handles.value_handle;
    params.p_data = (uint8_t *)magn_data;
    params.p_len = &len;
...
uint32_t ble_inert_notify_gyro(ble_inert_t * p_inert, Gyroscope *gyro_data)
{
	  ble_gatts_hvx_params_t params;
    uint16_t len = 6;			
    memset(&params, 0, sizeof(params));
    params.type = 0x01;
    params.handle = p_inert->gyro_char_handles.value_handle;
    params.p_data = (uint8_t *)gyro_data;
    params.p_len = &len;
